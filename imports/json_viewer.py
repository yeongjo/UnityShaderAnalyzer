...
class JsonView(QtWidgets.QWidget):
    ...
    def __init__(self, fpath):
        ...
        self.tree_widget.setHeaderLabels(["Variants"])
        ...
        self.tree_widget.itemClicked.connect(onClickEvent)
    ...
    def tree_add_row(self, key, val, tree_widget):
        ...
        if isinstance(val, dict) or isinstance(val, list):
            if isinstance(val, dict) and 'name' in val:
                key = str(key) + ': ' + val['name']
            text_list.append(key)
            row_item = QtWidgets.QTreeWidgetItem([key])
            self.recurse_jdata(val, row_item)
        else:
            if key == 'py/object' or key == 'name' or "OrderedDict" in str(val):
                return
            text_list.append(key)
            text_list.append(str(val))
            row_item = QtWidgets.QTreeWidgetItem([key])

        row_item2 = QtWidgets.QTreeWidgetItem([str(val)])
        row_item.addChild(row_item2)
